...
as_particle_type as_particle_type_from_msgpack(const uint8_t* packed,
...
// wire:
int as_bin_particle_modify_from_client(as_bin* b, cf_ll_buf* particles_llb,
		const struct as_msg_op_s* op);
int as_bin_particle_from_client(as_bin* b, cf_ll_buf* particles_llb,
		const struct as_msg_op_s* op);
...
int as_bin_bits_modify_from_client(as_bin* b, cf_ll_buf* particles_llb,
		struct as_msg_op_s* op);
int as_bin_bits_read_from_client(const as_bin* b, struct as_msg_op_s* op,
		as_bin* result);

// Different for HLL operations - we don't use the normal APIs and particle
// table functions.
int as_bin_hll_modify_from_client(as_bin* b, cf_ll_buf* particles_llb,
		struct as_msg_op_s* op, as_bin* rb);
...
int as_bin_cdt_modify_from_client(as_bin* b, cf_ll_buf* particles_llb,
		struct as_msg_op_s* op, as_bin* result);
int as_bin_cdt_read_from_client(const as_bin* b, struct as_msg_op_s* op,
		as_bin* result);
...
// particle table functions.
int as_bin_exp_modify_from_client(const struct as_exp_ctx_s* ctx, as_bin* b,
		cf_ll_buf* particles_llb, struct as_msg_op_s* op,
		const struct iops_expop_s* expop);
int as_bin_exp_read_from_client(const struct as_exp_ctx_s* ctx,
		struct as_msg_op_s* op, as_bin* rb);

// msgpack:
int32_t as_particle_size_from_msgpack(const uint8_t* packed,
		uint32_t packed_size);
bool as_bin_particle_from_msgpack(as_bin* b, const uint8_t* packed,
		uint32_t packed_size, void* mem);

// flat:
const uint8_t* as_bin_particle_from_flat(as_bin* b, const uint8_t* flat,
		const uint8_t* end);
...
int as_bin_bits_modify_tr(as_bin* b, const struct as_msg_op_s* msg_op,
		cf_ll_buf* particles_llb);
int as_bin_bits_read_tr(const as_bin* b, const struct as_msg_op_s* msg_op,
		as_bin* result);
...
// HLL:
int as_bin_hll_modify_tr(as_bin* b, const struct as_msg_op_s* msg_op,
		cf_ll_buf* particles_llb, as_bin* rb);
int as_bin_hll_read_tr(const as_bin* b, const struct as_msg_op_s* msg_op,
		as_bin* rb);
...
bool as_particle_geojson_match(as_particle* p, uint64_t cellid,
		geo_region_t region, bool is_strict);
bool as_particle_geojson_match_msgpack(msgpack_in* element, uint64_t cellid,
		geo_region_t region, bool is_strict);
char const* as_geojson_mem_jsonstr(const as_particle* p, size_t* p_jsonsz);
bool as_geojson_match(bool candidate_is_region, uint64_t candidate_cellid,
...
bool as_geojson_parse(const struct as_namespace_s* ns, const char* json,
		uint32_t jlen, uint64_t* cellid, geo_region_t* region);
bool as_geojson_to_particle(const char* json, uint32_t jlen, as_particle** pp);
bool as_bin_cdt_context_geojson_parse(as_bin* b);

// list:
void as_bin_particle_list_get_packed_val(const as_bin* b,
		struct cdt_payload_s* packed);

// map:
void as_bin_particle_map_get_packed_val(const as_bin* b,
		struct cdt_payload_s* packed);

int as_bin_cdt_modify_tr(as_bin* b, const struct as_msg_op_s* op,
		as_bin* result, cf_ll_buf* particles_llb);
int as_bin_cdt_read_tr(const as_bin* b, const struct as_msg_op_s* op,
		as_bin* result);
...
bool as_bin_cdt_get_by_context(const as_bin* b, const uint8_t* ctx,
...
{
	return as_bin_is_embedded_particle(b) ? (as_particle*)&b->particle
										  : b->particle;
}
...
as_bin* as_bin_get_w_len(struct as_storage_rd_s* rd, const uint8_t* name,
		size_t len);
as_bin* as_bin_get_live(struct as_storage_rd_s* rd, const char* name);
as_bin* as_bin_get_live_w_len(struct as_storage_rd_s* rd, const uint8_t* name,
		size_t len);
as_bin* as_bin_get_or_create(struct as_storage_rd_s* rd, const char* name);
as_bin* as_bin_get_or_create_w_len(struct as_storage_rd_s* rd,
		const uint8_t* name, size_t len);
void as_bin_delete(struct as_storage_rd_s* rd, const char* name);
void as_bin_delete_w_len(struct as_storage_rd_s* rd, const uint8_t* name,
		size_t len);
...
uint16_t plain_generation(uint16_t regime_generation,
		const struct as_namespace_s* ns);
void as_record_set_lut(as_record* r, uint32_t regime, uint64_t now_ms,
		const struct as_namespace_s* ns);
void as_record_increment_generation(as_record* r,
		const struct as_namespace_s* ns);
bool as_record_is_binless(const as_record* r);
bool as_record_is_live(const as_record* r);
int as_record_get_create(struct as_index_tree_s* tree, const cf_digest* keyd,
		struct as_index_ref_s* r_ref, struct as_namespace_s* ns);
int as_record_get(struct as_index_tree_s* tree, const cf_digest* keyd,
...
int as_record_fix_setless_tombstone(as_record* r, struct as_namespace_s* ns,
		const char* set_name, uint32_t len, bool apply_count_limit);

void as_record_drop_stats(as_record* r, struct as_namespace_s* ns);
void as_record_transition_stats(as_record* r, struct as_namespace_s* ns,
		const as_record* old_r);

void as_record_transition_set_index(struct as_index_tree_s* tree,
		struct as_index_ref_s* r_ref, struct as_namespace_s* ns,
		uint16_t n_bins, const as_record* old_r);
...
void as_record_advance_void_time(as_record* r, uint32_t record_ttl,
		uint64_t now, struct as_namespace_s* ns);
...
int as_record_apply(as_remote_record* rr, struct as_index_ref_s* r_ref,
		struct as_storage_rd_s* rd);

// For enterprise split only.
int record_resolve_conflict_cp(uint16_t left_gen, uint64_t left_lut,
...
int as_namespace_set_set_w_len(as_namespace* ns, const char* set_name,
		size_t len, uint16_t* p_set_id, bool apply_count_limit);
int as_namespace_get_create_set_w_len(as_namespace* ns, const char* set_name,
		size_t len, struct as_set_s** pp_set, uint16_t* p_set_id);
struct as_set_s* as_namespace_get_set_by_name(as_namespace* ns,
		const char* set_name);
struct as_set_s* as_namespace_get_set_by_id(const as_namespace* ns,
		uint16_t set_id);
struct as_set_s* as_namespace_get_record_set(const as_namespace* ns,
		const as_record* r);
void as_namespace_get_set_info(as_namespace* ns, const char* set_name,
		cf_dyn_buf* db);
void as_namespace_adjust_set_data_used_bytes(as_namespace* ns, uint16_t set_id,
		int64_t delta_bytes);
void as_namespace_release_set_id(as_namespace* ns, uint16_t set_id);
void as_namespace_get_hist_info(as_namespace* ns, char* set_name,
		char* hist_name, cf_dyn_buf* db);
