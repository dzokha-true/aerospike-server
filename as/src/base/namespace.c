...
as_namespace_create(const char* name)
{
	...
	ns->migrate_sleep = 1;
	ns->mrt_duration =
			0; // will be  set later to DEFAULT_MRT_DURATION (10 sec) if applicable and not explicitly configured
	...
	ns->n_tomb_raider_unmark_threads =
			0; // 0 means use one thread per device (default behavior)
...
append_set_props(as_set* p_set, cf_dyn_buf* db)
{
	...
	cf_dyn_buf_append_string(db, "sindexes=");
	cf_dyn_buf_append_uint32(db, p_set->n_sindexes);
	cf_dyn_buf_append_char(db, ':');

	cf_dyn_buf_append_string(db, "index_populating=");
